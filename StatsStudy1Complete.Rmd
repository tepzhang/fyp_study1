...
tmp2 <- lm(df$pers_neg_total ~ df$psqi_time, data = df)
summary(tmp2)
...
df_final <- df %>% 
  filter(!is.na(psqi_sq_c1),!is.na(pers_neg_total))

# linear regression for standardized coefficients
lm(scale(pers_neg_total) ~ scale(psqi_sq_c1) + scale(psqi_latency_c2) + scale(psqi_4),data = df_final) %>% summary()

lm(scale(pers_pos_total) ~ scale(psqi_sq_c1) + scale(psqi_latency_c2) + scale(psqi_4),data = df_final) %>% summary()
